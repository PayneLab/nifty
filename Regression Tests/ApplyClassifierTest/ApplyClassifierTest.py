#!/usr/bin/env python3
"""
Regression test for NIFty: feature selection + training + apply classifier.
"""
...
SELECTED_FEATURES_FILE = "selected_features.tsv"
...
def compare_tsv(expected_path: Path, actual_path: Path, label: str):
    ...
    print(f"  ✅ {label}: TSV matches expected")
...
def compare_models(expected_pkl: Path, actual_pkl: Path, label: str):
    ...
    # Common learned attributes (if present)
...
def main():
    ...
    expected_selected = test_dir / "expected_selected_features.tsv"
    ...
    with tempfile.TemporaryDirectory() as tmpdir_str:
        ...
        actual_selected = temp_dir / SELECTED_FEATURES_FILE
        compare_tsv(expected_selected, actual_selected, "Selected features")
        ...
        compare_tsv(expected_pred_classes, actual_pred_classes, "Predicted classes")

        print("\n✅ ApplyClassifierTest regression test PASSED — all outputs match expected.")
